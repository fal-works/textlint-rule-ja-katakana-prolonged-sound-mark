...
describe("lookup", () => {
  it("登録済みの語のカテゴリと MarkPolicy を返す", () => {
    const result = lookup("ユーザー");
    assert.deepEqual(result, { category: "er-or-ar", markPolicy: "requireMark" });
  });

  it("長音符トグルで検索できる", () => {
    // メモリ は ry カテゴリに登録、メモリー で検索してもヒット
    const result = lookup("メモリー");
    assert.deepEqual(result, { category: "ry", markPolicy: "requireNoMark" });
  });
  ...
  it("variants として登録された派生語も引ける（基幹語と同じ category/markPolicy）", () => {
  ...
  it("requireNoMark 側の variants も引ける", () => {
    ...
    assert.deepEqual(result, { category: "er-or-ar", markPolicy: "requireNoMark" });
  });
  ...
  it("allowBoth に移した偽同定防止語も独立エントリとして引ける", () => {
    ...
    assert.deepEqual(result, { category: "er-or-ar", markPolicy: "allowBoth" });
  });
  ...
  it("dict-ure 側の variants も引ける", () => {
    ...
    assert.deepEqual(result, { category: "ure", markPolicy: "requireNoMark" });
  });
});
