...
describe("validate", () => {
    ...
    const errors = validate(
      sources({
        src: {
          ...
          requireNoMark: [{ word: "シリアライザ", variants: ["デシリアライザ"] }],
          allowBoth: ["ブリーダー"],
        },
      }),
    );
  ...
  it("requireMark の付随語が長音符なしならエラー", () => {
    const errors = validate(
      sources({
        src: {
          requireMark: [{ word: "インストーラー", variants: ["アンインストーラ"] }],
        },
      }),
    );
  ...
  it("requireNoMark の付随語が長音符ありならエラー", () => {
    const errors = validate(
      sources({
        src: {
          requireNoMark: [{ word: "シリアライザ", variants: ["デシリアライザー"] }],
        },
      }),
    );
  ...
  it("variant の誤表記が基幹語の誤表記で終わらなければエラー", () => {
    const errors = validate(
      sources({
        src: {
          requireMark: [{ word: "インストーラー", variants: ["ダウンローダー"] }],
        },
      }),
    );
    assert.ok(
      errors.some((e) => /ダウンローダー.*誤表記.*インストーラー.*終わっていません/.test(e)),
    );
  });
  ...
  it("falsePositives の参照先が top-level 登録されていなければエラー", () => {
    const errors = validate(
      sources({
        src: {
          requireMark: [{ word: "リーダー", falsePositives: ["ブリーダー"] }],
        },
      }),
    );
  ...
  it("falsePositives の参照先のどの形も基幹語 wrongForm で終わらなければエラー", () => {
    const errors = validate(
      sources({
        a: {
          requireMark: [{ word: "リーダー", falsePositives: ["マネージャー"] }, "マネージャー"],
        },
      }),
    );
  ...
  it("cross-policy の falsePositives 参照は成立すれば正常", () => {
    const errors = validate(
      sources({
        a: {
          requireMark: [{ word: "リーダー", falsePositives: ["フリーダ"] }],
        },
        b: {
          requireNoMark: ["フリーダ"],
        },
  ...
  it("variant が他エントリと重複していればエラー", () => {
    ...
    assert.ok(errors.some((_e) => /重複.*アンインストーラー/));
  });
  ...
  it("ソース間でも単体エントリの冗長性を検知", () => {
    ...
    assert.ok(errors.some((e) => /アンインストーラー.*インストーラー.*後方一致/.test(e)));
  });
