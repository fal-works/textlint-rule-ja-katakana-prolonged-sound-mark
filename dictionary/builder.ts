...
export function validate(sources: Map<string, DictSource>): string[] {
    ...
    for (const entry of source.requireMark ?? []) {
      ...
      for (const variant of variantWords(entry)) {
        if (!variant.endsWith("ー")) {
          errors.push(
            `${name}: 「${variant}」（「${word}」の variants）が「ー」で終わっていません`,
          );
        }
      }
    }
  ...
  for (const x of topLevelEntries) {
    for (const y of topLevelEntries) {
      ...
      const overlaps = entryForms(x).some(
        (form) => form.length > yWrong.length && form.endsWith(yWrong),
      );
 ...
 * - `correctForms`: requireMark / requireNoMark の語と variants の正表記、 および allowBoth の両形
 *
 * いずれも文字数の降順でソートされる。
 */
 ...
 */
export function renderModule(forms: { wrongForms: string[]; correctForms: string[] }): string {
