...
/**
 * 辞書ソースに設定するルール。
 * 辞書生成時の検査方法と収録方法を決定する。
 *
 * - `"require-mark"` — 末尾長音符ありを正表記とする。辞書ソースでは正表記を管理する
 * - `"require-no-mark"` — 末尾長音符なしを正表記とする。辞書ソースでは正表記を管理する
 * - `"no-check"` — 登録語の末尾長音符バリデーションを行わない。辞書ソースでは正表記を管理する
 * - `"allow-both"` — 末尾長音符の有無を両方とも許容し、いずれも誤表記として扱うことを禁止する
 *
